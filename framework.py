...
#
# def oldcrossover(self):
#                    # randomly choose two chromosomes from population, with P(crossover_prob) perform crossover between two chromosomes
#                    # repeat until all chromosomes have been chosen
#                    parent_pool = copy.deepcopy(self.population)
#                    while len(parent_pool) > 1:
#                    father = parent_pool.pop(0)
#                    mother = random.choice(parent_pool)
#                    # print("mother = father", father == mother)
#                    # print(f"pool: {parent_pool}")
#                    parent_pool.remove(mother)
#                    if np.random.rand() < self.crossover_prob:
#                    father_index = self.population.index(father)
#                    mother_index = self.population.index(mother)
...
#                    crossover_length = random.randint(0, father.chrom_length)
#                    father[:crossover_length] = mother_copy[:crossover_length]
#                    mother[:crossover_length] = father_copy[:crossover_length]
#                    self.population[father_index] = father
#                    self.population[mother_index] = mother
#                    # son = ListChromosome.empty(father.chrom_length, father.valid_values)
#                    # daughter = ListIndividual.empty(father.chrom_length, father.valid_values)
#                    # perform crossover
#                    # crossover_length = random.randint(0, father.chrom_length)
#                    # father[crossover_length:] = mother_copy[crossover_length:]
#                    # mother[crossover_length:] = father_copy[crossover_length:]
...
#                      daughter[:crossover_length] = mother[:crossover_length]
#                      daughter[crossover_length:] = father[crossover_length:]
#                      # son.extend(mother[crossover_length:])
#                      # daughter = mother[:crossover_length]
#                      # daughter.extend(father[crossover_length:])
...
#                      self.population[mother_index] = daughter
#
# class oldMatchStringGA(AbstractGA):
#     def __init__(self, target_string, valid_values, population_size, crossover_prob, mutation_prob, tournament_size):
#         # NOTE target_string should be a list
#         super(MatchStringGA, self).__init__()
#         chrom_length = len(target_string)
#         self.population = [ListChromosome.random(chrom_length, valid_values=valid_values) for _ in range(population_size)]
#         self.crossover_prob = crossover_prob
#         self.mutation_prob = mutation_prob
#         self.tournament_size = tournament_size
#         self.target_string = target_string
...
#             chromosome = self.population[i]
#             if np.random.rand() < self.mutation_prob:
#                 new_chromosome = ListChromosome.empty(chromosome.chrom_length, chromosome.valid_values)
#                 # select gene to flip
#                 gene_choice = random.choice([j for j in range(len(chromosome))])
#                 # TODO maybe generalize this
#                 new_chromosome[:] = chromosome[:]
#                 if chromosome[gene_choice] == 0:
#                     new_chromosome[gene_choice] = 1
#                 else:
#                     new_chromosome[gene_choice] = 0
#
#                 self.population[i] = new_chromosome
#
#     def select(self):
#         for chromosome in self.population:
#             # make sure fitness score is set
#             assert chromosome.fitness_score
#
#         new_population = []
#         # tournament selection with replacement
#         for tourney_counter in range(len(self.population)):
#             contestants = random.sample(self.population, self.tournament_size)
#             contestants.sort(key=lambda x: x.fitness_score, reverse=True)
#             winner = contestants[0]
#             new_population.append(winner)
...
#
#
# class OldDeceptiveMaxOneGA(MatchStringGA):
#     def __init__(self, string_length, population_size, crossover_prob, mutation_prob, tournament_size):
#         super(DeceptiveMaxOneGA, self).__init__(target_string=[1 for _ in range(string_length)],
#                                                 population_size=population_size,
#                                                 crossover_prob=crossover_prob,
#                                                 mutation_prob=mutation_prob,
#                                                 tournament_size=tournament_size)
...
#
# @staticmethod
# def empty(population_size, chrom_length, values):
#     # returns empty population list
#     population = ListPopulation()
#     population.population_size = population_size
#     population.chrom_length = chrom_length
#     population.values = values
#     return population
#
# @staticmethod
# def random(population_size, chrom_length, values):
#                                               # create population with random chromosomes
#                                               population = ListPopulation()
#                                               for i in range(population_size):
#                                               population.append([random.choice(values) for _ in range(chrom_length)])
#                                               population.population_size = population_size
#                                               population.chrom_length = chrom_length
#                                               population.values = values
#                                               return population
...
#     # TODO concatenate or extend
#     # TODO allow for slicing
#     def __init__(self):
...
#         raise NotImplementedError
#
#     # def __init__(self, population_size, chrom_length, values):
#     #     super(ListPopulation, self).__init__()
#     #     self.population_size = population_size
#     #     self.chrom_length = chrom_length
#     #     for i in range(population_size):
#     #         self.append([random.choice(values) for _ in range(chrom_length)])
#
#
